...
hist2 <- ggplot()+
        ...
        geom_histogram(data = df_by_day, aes(steps_by_day, fill = "red3"), alpha = 0.5) +
        ylab("Count (number of days)")+
        xlab("Number of steps (unit)")+
        labs(title = "Histogram of the total steps taken by day.",
        ...
        geom_vline(xintercept = median(df_by_day$steps_by_day, na.rm = T),
                   color = "red3", lwd = 1.2, alpha = 0.5)+
        scale_fill_manual(values = c("red3", "steelblue"), labels = c( "NA's", "No NA's"),
                          name = NULL) +
        theme_bw()
hist2
```
